...
- проектные проверочные агенты Codex, Claude Code, GitHub Copilot и OpenCode, а также агенты `truth-doc-writer` с явно выданной областью записи под `.codex/agents/`, `.claude/agents/`, `.github/agents/` и `.opencode/agents/` для аудитов с subagents, которыми владеет рабочий процесс, и для документных шардов, выданных родительским агентом
