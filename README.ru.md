...
**Ваши агенты пишут код. Truthmark поддерживает ориентированную на людей документацию, проверяемую в Git.**
...
ИИ-агенты для разработки могут менять репозиторий быстрее, чем люди успевают выравнивать его документацию.
...
- каждой сессии агента приходится заново открывать истину репозитория

Truthmark превращает эту хрупкую истину репозитория в зафиксированную инфраструктуру репозитория.
...
Укажите имя функции, пути к коду, пути к тестам или желаемую область truth-документов. В OpenCode-подобных хостах тот же workflow вызывается как `/skill truthmark-document ...`; в Gemini CLI используйте `/truthmark:document ...`.

Для большого repo, где все еще есть один широкий placeholder-маршрут, сначала запустите Truth Structure, а затем вызывайте Truth Document по одному ограниченному feature или area за раз.
...
```text
/truthmark-realize реализуй docs/truthmark/truth/authentication/session-timeout.md в коде
```
...
```text
/truthmark-document задокументируй реализованный password reset flow в docs/truthmark/truth/authentication
```
...
```text
/truthmark-realize реализуй docs/truthmark/truth/billing/invoice-retry-policy.md в коде
```
...
- [Установленные workflows](docs/truthmark/truth/workflows/overview.md)
