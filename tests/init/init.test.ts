...
describe("runInit", () => {
  ...
  it("creates the Truthmark scaffold in an empty repository", async () => {
    ...
    try {
      ...
      const syncValidateReportScript = await repo.readFile(
        ".codex/skills/truthmark-sync/scripts/validate-sync-report.mjs",
      );
      ...
      expect(syncHelperManifest).toContain("runner: node>=20");
      expect(syncHelperManifest).toContain(
      ...
      expect(syncHelperPolicy).toContain("Optional helper scripts");
      expect(syncHelperPolicy).toContain("manual fallback");
      expect(syncHelperPolicy).toContain("Helper scripts:");
      expect(syncValidateReportScript).toContain("Generated by Truthmark");
      expect(syncValidateReportScript).toContain("process.argv");
  ...
  it("scaffolds typed truth-doc defaults under docs/truth", async () => {
    ...
    try {
      ...
      expect(
        await repo.readFile("docs/templates/architecture-doc.md"),
      ...
      expect(
        await repo.readFile("docs/templates/test-behavior-doc.md"),
...
`,
      ...
      expect(
        await repo.readFile(
          ".codex/skills/truthmark-realize/support/report-template.md",
        ),
...
`,
      ...
      await fs.rm(
        path.join(repo.rootDir, "docs/truthmark/areas/repository.md"),
      );
...
`,
      ...
      expect(await repo.readFile("AGENTS.md")).toContain(
