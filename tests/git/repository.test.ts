...
describe("resolveWorktreePath", () => {
  it("keeps resolved paths inside the active checkout", async () => {
    ...
    try {
      ...
      expect(
        resolveWorktreePath(repository, "docs/truthmark/routes/areas.md"),
      ).toBe(
  ...
  it("rejects symlinked paths that resolve outside the active checkout", async () => {
    ...
    try {
      const outsidePath = path.resolve(
        repo.rootDir,
        "..",
        "truthmark-worktree-outside.txt",
      );
      ...
      await fs.promises.symlink(
        outsidePath,
        path.join(repo.rootDir, "docs", "outside-link.txt"),
      );

      const repository = await getGitRepository(repo.rootDir);

      expect(() =>
        resolveWorktreePath(repository, "docs/outside-link.txt"),
      ).toThrow("must stay inside the active worktree");
    } finally {
      await fs.promises.rm(
        path.resolve(repo.rootDir, "..", "truthmark-worktree-outside.txt"),
        {
          force: true,
        },
      );
  ...
  it("rejects missing leaves under parent symlinks that point outside the active checkout", async () => {
    ...
    try {
      const outsideDir = path.resolve(
        repo.rootDir,
        "..",
        "truthmark-worktree-outside-dir",
      );
      ...
      await fs.promises.symlink(
        outsideDir,
        path.join(repo.rootDir, "docs", "outside-dir"),
      );

      const repository = await getGitRepository(repo.rootDir);

      expect(() =>
        resolveWorktreePath(repository, "docs/outside-dir/new.txt"),
      ).toThrow("must stay inside the active worktree");
    } finally {
      await fs.promises.rm(
        path.resolve(repo.rootDir, "..", "truthmark-worktree-outside-dir"),
        {
          force: true,
          recursive: true,
        },
