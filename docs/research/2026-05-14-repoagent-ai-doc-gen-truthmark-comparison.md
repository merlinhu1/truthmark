...
- Documentation generation: `DocumenterAgent` reads the available `.ai/docs/*.md` files and optionally an existing README, then writes `README.md` using a Pydantic output model. `AIRulesGeneratorAgent` reuses the same pattern for `CLAUDE.md`, `AGENTS.md`, and editor assistant rules.
