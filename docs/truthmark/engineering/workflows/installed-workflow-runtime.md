...
The source manifest and renderers generate a canonical repository-local workflow package under `.truthmark/agent/`. `.truthmark/agent/manifest.json` records deterministic repository-relative package paths and sha256 hashes, and `.truthmark/agent/workflows/<workflow>/` owns each workflow entrypoint, procedure, report template, subagent/lease guidance, and helper metadata. Host-specific skills, prompts, commands, subagents, and managed instruction blocks are projections of that package, not separate workflow authority. Generated host skill directories remain native skill packages: their `SKILL.md` files keep package-local `support/procedure.md`, `support/report-template.md`, subagent/lease guidance, helper manifests, and helper policy files so hosts that package skill-directory resources do not depend on arbitrary cross-repository file reads. GitHub Copilot prompts, Gemini commands, and top-level managed instruction blocks stay thin and point to workflow entrypoints rather than embedding full workflow bodies. Generated-surface checks report missing canonical package files, stale package/manifest content, and missing or stale host-native skill package files.
...
- Decision (2026-06-17): `.truthmark/agent/` is the canonical repository-local Truthmark workflow package; host skill directories are native generated package projections with colocated resources, while prompt/command surfaces stay thin adapters with freshness diagnostics instead of independent workflow authorities.
