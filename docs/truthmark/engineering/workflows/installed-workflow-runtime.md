...
The source manifest and renderers generate a canonical repository-local workflow package under `.truthmark/agent/`. `.truthmark/agent/manifest.json` records deterministic repository-relative package paths and sha256 hashes, and `.truthmark/agent/workflows/<workflow>/` owns each workflow entrypoint, procedure, report template, subagent/lease guidance, and helper metadata. Host-specific skills, prompts, commands, subagents, and managed instruction blocks are projections of that package, not separate workflow authority. `.agents` and `.opencode` workflow skill files are thin adapters that point to canonical package files and avoid full workflow procedure/report bodies. Hosts that still need inline files use explicit `expanded-adapter` provenance comments with canonical path, canonical hash, and generated hash so stale copied bodies are detectable. Generated-surface checks report missing canonical package files, stale package/manifest content, adapter-only files that duplicate workflow body markers, missing canonical references, and stale expanded-adapter hashes.
...
- Decision (2026-06-17): `.truthmark/agent/` is the canonical repository-local Truthmark workflow package; host workflow files are adapter or expanded-adapter projections with freshness diagnostics instead of independent workflow authorities.
