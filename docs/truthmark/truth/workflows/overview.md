...
No CLI command or helper output is a required prerequisite before an agent can act. Agents inspect the checkout directly, apply workflow boundaries from the committed surfaces plus any available live read-only workflow contract, update only workflow-allowed files, and report what changed. When the live workflow contract is missing, Sync, Document, Structure, and Realize may spend extra context on bounded manual fallback, but the fallback is limited to the current focus, changed files, configured route hints, named workflow support files, and direct evidence needed for the workflow. Agents must block on ambiguity instead of reading every support file, route file, or truth doc until they feel certain.
...
Every public generated workflow entrypoint, GitHub Copilot prompt, and Gemini command includes the live workflow preflight before workflow-specific procedure text. The preflight uses one `workflow instructions` call with the canonical full workflow ID, tells agents to inspect `data.workflowState.applicability.state`, `data.workflowState.actionContext.allowedWritePaths`, `data.workflowState.actionContext.forbiddenWritePaths`, `data.workflowState.actionContext.stopConditions`, `data.instructions.stopConditions`, helper validation commands, and report template/final-report fields, and requires a visible skipped status when the local workflow CLI is unavailable. For Sync, Document, Structure, and Realize, skipped-live fallback is intentionally bounded: agents may read only the current entrypoint's named support files plus focused route/truth/code evidence, and they must block as ambiguous rather than scan every support file, every route file, or every truth doc by default. Generated support files remain subordinate references and do not duplicate the preflight.
