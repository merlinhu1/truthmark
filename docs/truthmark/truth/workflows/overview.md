...
Generated workflow entrypoints, prompts, and commands do not ask agents to load live workflow preflight JSON before acting. Checked-in generated prose, progressive-disclosure support files, route files, truth docs, and direct checkout inspection are the normal workflow contract. The local CLI remains an optional focused validation tool for commands such as `truthmark check --json`, `truthmark index --json`, and declared helper validators; `truthmark workflow status --workflow <id> --json` is retained for status-only/debug inspection, not as a generated workflow prerequisite.
