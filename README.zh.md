...
提供功能名、代码路径、测试路径或目标事实文档区域。在 OpenCode 风格的宿主中，同一个工作流用 `/skill truthmark-document ...` 调用；在 Gemini CLI 中，使用 `/truthmark:document ...`。
...
```text
/truthmark-realize 将 docs/truthmark/truth/authentication/session-timeout.md 实现到代码中
```
...
```text
/truthmark-document 在 docs/truthmark/truth/authentication 下记录已实现的密码重置流程
```
...
```text
/truthmark-realize 将 docs/truthmark/truth/billing/invoice-retry-policy.md 实现到代码中
```
...
- [已安装工作流](docs/truthmark/truth/workflows/overview.md)
